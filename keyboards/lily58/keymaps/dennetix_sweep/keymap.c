...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    */
    ...
    _, KC_Q,     LGUI_T(KC_W), KC_F,         KC_P,         KC_B,                                  KC_J,                KC_L,         CKC_UMLU,     LGUI_T(CKC_Y), CKC_SCLN, _,
    _, CKC_UMLA, LALT_T(KC_R), LSFT_T(KC_S), LCTL_T(KC_T), KC_G,                                  KC_M,                RCTL_T(KC_N), RSFT_T(KC_E), LALT_T(KC_I),  CKC_UMLO, _,
    _, CKC_Z,    KC_X,         KC_C,         KC_D,         KC_V,              _,          _,      KC_K,                KC_H,         CKC_COMM,     CKC_DOT,       CKC_SLSH, _,
                                                   _, _,   LT(_LEFT, KC_TAB), CKC_SFTENT, KC_SPC, LT(_RIGHT, KC_BSPC), _, _
    ),
    ...
    */
    ...
    KC_LSFT,  CKC_Z,    KC_X,         KC_C,         KC_V,         KC_B,      CKC_LBRC, CKC_RBRC, KC_N,       KC_M,         CKC_COMM,     CKC_DOT,      CKC_SLSH, KC_RSFT,
                                      KC_LGUI,      MO(_ADJUST),  MO(_LEFT), KC_ENT,   KC_SPC,   MO(_RIGHT), TG(_GAMING),  TG(_QWERTY)
    ),
    ...
    */
    [_LEFT] = LAYOUT(+
    _, _,       _,       _,       _,       _,           _,     _,       _,       _,       _,        _,
    _, KC_1,    CKC_2,   CKC_3,   KC_4,    KC_5,        CKC_6, CKC_7,   CKC_8,   CKC_9,   CKC_0,    _,
    _, KC_CAPS, KC_MPRV, KC_MPLY, KC_MNXT, _,           _,     KC_LEFT, KC_DOWN, KC_UP,   KC_RIGHT, _,
    _, _,       _,       _,       _,       _,     _, _, _,     KC_PSCR, KC_PGDN, KC_PGUP, _,        _,
                         _,       _,       _,     _, KC_ESC, LT(_ADJUST, KC_DEL), _,       _
    ),
    ...
    */
    [_RIGHT] = LAYOUT(
    _, _,       _,        _,        _,        _,                 _,        _,        _,        _,        _,        _,
    _, KC_EXLM, CKC_AT,   CKC_HASH, KC_DLR,   KC_PERC,           CKC_CIRC, CKC_AMPR, CKC_ASTR, CKC_GRV,  CKC_TILD, _,
    _, _,       CKC_LBRC, CKC_LCBR, CKC_LPRN, CKC_SZ,            _,        CKC_MINS, CKC_EQL,  CKC_PIPE, CKC_QUOT, _,
    _, _,       CKC_RBRC, CKC_RCBR, CKC_RPRN, CKC_EURO,    _, _, _,        CKC_UNDS, CKC_PLUS, CKC_BSLS, CKC_DQUO, _,
                          _,        _,        MO(_ADJUST), _, _, _,        _,        _
    ),
    ...
    */
    ...
    _, _,     _,     _,     _,     _,           _,     _,           _,          _,     KC_F11, _,
    _, _,     _,     _,     _,     _,     _, _, _,     TG(_GAMING), TG(_QWERTY), _,     KC_F12, _,
                            _,     _,     _, _, _, _,  _,           _
    )
};
...
// CKC_SFTETN
bool keys_pressed_while_lshift = false;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (physical_lshift_down && keycode != CKC_SFTENT && record->event.pressed) {
        keys_pressed_while_lshift = true;
    }
    ...
    case CKC_SFTENT:
        if (record->event.pressed) {
            register_code(KC_LSFT);
            physical_lshift_down = true;
            lshift_down = true;
            keys_pressed_while_lshift = false;
            lshift_timer = timer_read();
        } else {
            unregister_code(KC_LSFT);
            physical_lshift_down = false;
            lshift_down = false;
            if (!keys_pressed_while_lshift && timer_elapsed(lshift_timer) <= ENTER_TAPPING_TERM) {
                tap_code(KC_ENT);
            }
        }
        break;
    ...
    case CKC_GRV:
        ...
        break;
    case CKC_DQUO:
        handle_key(keycode, record->event.pressed, KC_2, KC_2, true, true, false, false);
        break;
    case CKC_TILD:
        handle_key(keycode, record->event.pressed, KC_RBRC, KC_RBRC, false, false, true, true);
        break;
